...
class RankFollow(View):
    ...
    def get(self, request):
        datas = []
        users = UserProfile.objects.order_by('fan_nums')        
        for user in users:
            images = ImageModel.objects.order_by('like_nums')[:2]
            data = {
                        "id": user.id,
                        "username": user.username,
                        "email": user.email,
